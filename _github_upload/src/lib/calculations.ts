...
export function normalizeSettings(settings: PortfolioSettings): PortfolioSettings {
  return {
    ...
    goalAccountValue: safeNumber(settings.goalAccountValue ?? 4424),
    goalMonthlyDeposit: safeNumber(settings.goalMonthlyDeposit ?? 500),
    goalTargetValue: safeNumber(settings.goalTargetValue ?? 25000),
    goalAnnualReturn: safeNumber(settings.goalAnnualReturn ?? 10),
    mrvlGoalCurrentShares: safeNumber(settings.mrvlGoalCurrentShares ?? 0)
  };
}
